...
const RunsScreen: React.FC<RunsScreenProps> = ({ navigation, route }) => {
  ...
  const onRefresh = React.useCallback(async () => {
    ...
    try {
      await refetch();
      showToast('Runs refreshed', 'success');
    } catch (error) {
      showToast('Failed to refresh runs', 'error');
    } finally {
      setRefreshing(false);
    }
  }, [refetch, showToast]);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]} edges={['top']}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            icon={searchQuery || selectedStatus !== 'ALL' ? 'search-off' : 'play-circle-outline'}
            title={searchQuery || selectedStatus !== 'ALL' ? 'No Matching Runs' : 'No Runs Yet'}
            description={
              searchQuery || selectedStatus !== 'ALL'
                ? 'Try adjusting your search or filters'
                : 'Runs will appear here once jobs are executed'
            }
          />
        }
      />
