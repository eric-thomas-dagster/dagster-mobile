...
const AutomationsScreen: React.FC<AutomationsScreenProps> = ({ navigation, route }) => {
  ...
  const onRefresh = React.useCallback(async () => {
    ...
    try {
      ...
      showToast('Automations refreshed', 'success');
    } catch (error) {
      console.error('AutomationsScreen - Refresh error:', error);
      showToast('Failed to refresh automations', 'error');
    } finally {
      setRefreshing(false);
    }
  ...
  }, [refetchRepositories, client, showToast]);
        ...
        ListEmptyComponent={
          repositoriesError ? (
            <EmptyState
              ...
              description="There was an error loading repositories. Please try refreshing."
              actionLabel="Refresh"
              onActionPress={onRefresh}
            />
          ) : (
            <EmptyState
              icon={viewMode === 'sensors' ? 'radar' : 'schedule'}
              title={`No ${viewMode === 'sensors' ? 'Sensors' : 'Schedules'} Found`}
              description={
                viewMode === 'sensors'
                  ? 'Sensors monitor external events and trigger jobs when conditions are met'
                  : 'Schedules run jobs at specified time intervals'
              }
            />
          )
        }
